...
class AdminPage:
    ...
    def login(self):
        ...
        # 数据库操作 查询管理员表
        db = pymysql.connect(host='localhost', port=3306, db='student', user='root', password='Dou23507851') # 打开数据库连接
...
class AdminManage:
    def __init__(self, parent_window):
        ...
        self.age = []
        # 打开数据库连接
        db = pymysql.connect(host='localhost', port=3306, db='student', user='root', password='Dou23507851')
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        ...
        self.right_top_button4 = ttk.Button(self.frame_right_top, text='查询选中学生信息', width=20,
                                            command=self.search_row)

        # 位置设置
        self.right_top_title.grid(row=1, column=0, pady=10)
        self.right_top_button1.grid(row=2, column=0, padx=15, pady=5)
        self.right_top_button2.grid(row=3, column=0, padx=15, pady=5)
        self.right_top_button3.grid(row=4, column=0, padx=15, pady=5)
        self.right_top_button4.grid(row=5, column=0, padx=15, pady=5)
    ...
    def updata_row(self):
        ...
        if res == True:
            if self.var_id.get() == self.row_info[0]:  # 如果所填学号 与 所选学号一致
                # 打开数据库连接
                db = pymysql.connect(host='localhost', port=3306, db='student', user='root',
                                        password='Dou23507851')
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
    ...
    def search_row(self):#查询
        res = messagebox.askyesnocancel('警告！', '是否查询学生信息？')
        if res == True:
            if str(self.var_id.get()) in self.id:
                print(self.var_id.get())
                print(self.id)
                db = pymysql.connect(host='localhost', port=3306, db='student', user='root', password='Dou23507851')
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
                sql_search = "SELECT * FROM student_k WHERE id = '%s'"%(self.var_id.get())  # SQL 插入语句
                try:
                    cursor.execute(sql_search)  # 执行sql语句
                    db.commit()  # 提交到数据库执行
                    messagebox.showinfo('提示！', '查询成功！学生表中存在这名学生。')
                except:
                    print("Error: unable to fetch data")
                    messagebox.showinfo('警告！', '查询失败！学生表中不存在这名学生。')
                db.close()  # 关闭数据库连接
            else:
                print("Error: unable to fetch data")
                messagebox.showinfo('警告！', '查询失败！学生表中不存在这名学生。')
...
class Changekey:
    ...
    def login(self):
        ...
        stu_pass = None
        # 数据库操作 查询管理员表
        db = pymysql.connect(host='localhost', port=3306, db='student', user='root', password='Dou23507851')  # 打开数据库连接   已经修正删除多余空格
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
...
class AboutPage:
    def __init__(self, parent_window):
        ...
        Label(self.window, text='初始密码和学号相同', font=('Verdana', 18)).pack(pady=5)
...
class AdminChange:
    def __init__(self, parent_window):
        ...
        Button(self.window, text="确定修改", width=8, font=tkFont.Font(size=12), command=self.change).pack(pady=40)
        Button(self.window, text="返回首页", width=8, font=tkFont.Font(size=12), command=self.back).pack()
